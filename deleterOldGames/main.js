        ...
        if (game1Ids){
            for (let game1Id of game1Ids){
                ...
                if ((timeFrame1.finTime - timeFrame1.startTime) / 60000 >= TIMEDELTA){    
                    const game2Ids = await db('games').select('id').where('sportKey', sportKey).where('isLive', game1Id.isLive).whereNot('id', game1Id.id).whereNot('bookieKey', game1Id.bookieKey); // получение списка id2
